...
Feature: User Login API
  ...
  Scenario Outline: Verify user login with different credentials
  	...
  	When I set Api
    Given the API endpoint "https://api.example.com/login" is available
    When I send a POST request with the following JSON payload:
      """
      {
        "username": "<username>",
        "password": "<password>"
    ...
    Examples:
      ...
      | testuser  | Test@123  | 200         | Login successful       |
